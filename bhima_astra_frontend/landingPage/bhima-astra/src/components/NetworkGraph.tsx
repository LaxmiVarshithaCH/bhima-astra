import React from 'react';
import { motion } from 'framer-motion';

const NetworkGraph: React.FC = () => {
  return (
    <div className="py-20 px-4 bg-gradient-to-b from-black to-charcoal">
      <div className="max-w-7xl mx-auto">
        <motion.div
          className="text-center mb-16"
          initial={{ opacity: 0, y: 50 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.8 }}
        >
          <div className="relative">
            {/* Stage indicator */}
            <div className="text-left mb-4">
              <span className="text-gray-500 text-sm">STAGE 03</span>
              <h2 className="text-white text-lg font-semibold">NETWORK GRAPH</h2>
            </div>
            
            {/* Large background number */}
            <div className="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none">
              <span className="text-9xl font-black text-white">03</span>
            </div>
            
            {/* Main content */}
            <div className="relative z-10">
              <h2 className="text-4xl md:text-6xl font-bold mb-6">
                <span className="text-white">NETWORK </span>
                <span className="text-white">GRAPH</span>
              </h2>
              <p className="text-xl text-gray-300 mb-8 max-w-3xl mx-auto leading-relaxed">
                Graph neural networks map transaction relationships. Identifies fraud rings, 
                money mules, synthetic identities.
              </p>
              
              {/* Tags */}
              <div className="flex flex-wrap justify-center gap-4">
                <motion.div
                  className="px-6 py-3 bg-gray-900 rounded-full text-white font-semibold border border-gray-700"
                  whileHover={{ scale: 1.05, backgroundColor: "#2a2a2a" }}
                  whileTap={{ scale: 0.95 }}
                >
                  45M5
                </motion.div>
                <motion.div
                  className="px-6 py-3 bg-gray-900 rounded-full text-white font-semibold border border-gray-700"
                  whileHover={{ scale: 1.05, backgroundColor: "#2a2a2a" }}
                  whileTap={{ scale: 0.95 }}
                >
                  GRAPH DB
                </motion.div>
                <motion.div
                  className="px-6 py-3 bg-gray-900 rounded-full text-white font-semibold border border-gray-700"
                  whileHover={{ scale: 1.05, backgroundColor: "#2a2a2a" }}
                  whileTap={{ scale: 0.95 }}
                >
                  6.2M NODES
                </motion.div>
              </div>
            </div>
          </div>
        </motion.div>

        {/* Visual representation */}
        <motion.div
          className="mt-16"
          initial={{ opacity: 0, scale: 0.8 }}
          whileInView={{ opacity: 1, scale: 1 }}
          viewport={{ once: true }}
          transition={{ duration: 0.8, delay: 0.3 }}
        >
          <div className="glass p-8 rounded-3xl">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="text-center">
                <div className="w-20 h-20 mx-auto mb-4 bg-gradient-to-br from-purple-500 to-purple-700 rounded-full flex items-center justify-center">
                  <div className="w-10 h-10 bg-white rounded-full"></div>
                </div>
                <h3 className="text-xl font-bold text-white mb-2">Node Analysis</h3>
                <p className="text-gray-400">Deep relationship mapping between entities</p>
              </div>
              <div className="text-center">
                <div className="w-20 h-20 mx-auto mb-4 bg-gradient-to-br from-blue-500 to-blue-700 rounded-full flex items-center justify-center">
                  <div className="w-10 h-10 bg-white rounded-full"></div>
                </div>
                <h3 className="text-xl font-bold text-white mb-2">Pattern Detection</h3>
                <p className="text-gray-400">Advanced fraud ring identification</p>
              </div>
              <div className="text-center">
                <div className="w-20 h-20 mx-auto mb-4 bg-gradient-to-br from-green-500 to-green-700 rounded-full flex items-center justify-center">
                  <div className="w-10 h-10 bg-white rounded-full"></div>
                </div>
                <h3 className="text-xl font-bold text-white mb-2">Real-time Processing</h3>
                <p className="text-gray-400">Instant threat detection and response</p>
              </div>
            </div>
          </div>
        </motion.div>
      </div>
    </div>
  );
};

export default NetworkGraph;
